...
/**
 *  @brief:     Returns max total price to spend, while being within budget.
 *  @param:     vector<int> keyboards : Each index is the price of a different 
 *                      keyboard.
 *              vector<int> drives : Each index is the price of a different 
 *                      drive.
 *              int b : The budget.
 *  @runtime:   O(N^2)
 */
int getMoneySpent(vector<int> keyboards, vector<int> drives, int b)
{
    ...
    for (auto keyboard : keyboards)
    {
        ...
        {
            int price_of_both = keyboard + drive;
            if (price_of_both > max_price && price_of_both <= b)
            {
                max_price = price_of_both;
            }
        }
    }
